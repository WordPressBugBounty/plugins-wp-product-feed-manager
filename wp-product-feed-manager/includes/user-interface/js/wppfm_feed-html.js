...
function wppfm_valueEditor( rowId, sourceLevel, valueEditorLevel, valueObject ) {
	...
	html += '<div class="wppfm-attribute-column wppfm-colw wppfm-col20w" id="wppfm-attribute-title-column">&nbsp</div>';
	html += '<div class="wppfm-attribute-column source-selector wppfm-colw wppfm-col30w" id="source-select-' + rowId + '-' + sourceLevel + '">';
	html += valueSelector;
	...
	html += '</span>';
	html += '</div>';
	html += '<div class="wppfm-attribute-column condition-selector wppfm-colw" id="condition-data-' + rowId + '-' + sourceLevel + '">';
...
function wppfm_forAllProductsAtChangeValuesSelector( rowId, sourceLevel, valueEditorLevel, displayStatus ) {
	...
	return '<div class="wppfm-colw wppfm-col30w allproducts" id="value-editor-input-query-span-' + rowId + '-' + sourceLevel + '-0" style="display:' + displayStatus + ';">'
		+ wppfm_feed_settings_form_vars.all_other_products.replace( '%other%', other_val )
...
function wppfm_valueOptionsSingleInputValue( rowId, sourceLevel, valueEditorLevel, optionsSelectorValue ) {
	...
	return ' <input type="text" onchange="wppfm_valueInputOptionsChanged(' + rowId + ', ' + sourceLevel
		+ ', ' + valueEditorLevel + ')" class="wppfm-value-option-input" id="value-options-input-' + rowId + '-' + sourceLevel + '-' + valueEditorLevel + '" value="' + optionsSelectorValue + '">';
}
...
function wppfm_changeValueCntrl( rowId, conditionLevel, valueEditorLevel, selectedValue ) {
	...
	var htmlCode = '<select class="select-value-options" id="value-options-'
		+ rowId + '-' + conditionLevel + '-' + valueEditorLevel + '" onchange="wppfm_valueOptionChanged(' + rowId + ', ' + conditionLevel + ', 0)"> ';
...
function wppfm_inputFieldCntrl( rowId, sourceLevel, sourceValue, staticValue, advisedSource, combinedValue, isCustom ) {
	...
	return '<div class="wppfm-select-control">' + prefix + '<select class="wppfm-select-control input-select wppfm-select2-selector" id="input-field-cntrl-' + rowId + '-' + sourceLevel
...
function wppfm_combinedInputFieldCntrl( rowId, sourceLevel, combinedLevel, selectedValue, fieldName, channel ) {
	...
	return '<select class="wppfm-select-control input-select align-left wppfm-select2-selector" id="combined-input-field-cntrl-' + rowId + '-' + sourceLevel + '-' + combinedLevel
...
function wppfm_conditionFieldCntrl( id, sourceLevel, conditionLevel, subConditionLevel, identifier, selectedValue, onChange ) {
	...
	return '<select class="wppfm-select-control input-select wppfm-select2-selector" id="' + identifier + '-' + id + '-' + sourceLevel + '-' + conditionLevel + subConditionLevelString + '"' + onChangeFunction + '> '
		+ '<option value="select">-- ' + wppfm_feed_settings_form_vars.select_a_source_field + ' --</option>'
		+ emptyOption
