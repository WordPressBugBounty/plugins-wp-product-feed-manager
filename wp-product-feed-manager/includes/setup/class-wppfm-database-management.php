...
if ( ! class_exists( 'WPPFM_Database_Management' ) ) :
	...
	class WPPFM_Database_Management {
		...
		public function verify_db_version() {
			$actual_db_version = get_option('wppfm_db_version') ?: $this->get_current_db_version();
		...
		/**
		 * Adds the custom capabilities edit_feeds and delete_feeds to the Editor role and above.
		 *
		 * @since 3.9.0
		 * @return void
		 */
		private function add_custom_capabilities() {
			...
			if ( ! $editor_role->has_cap( 'edit_feeds' ) ) { // Assuming that if the edit does not have the edit_feeds capability, all custom capabilities need to be (re)set
				$admin_role->add_cap( 'edit_feeds' );
				$admin_role->add_cap( 'delete_feeds' );

				$editor_role->add_cap( 'edit_feeds' );
				$editor_role->add_cap( 'delete_feeds' );
			}
		}
		...
		private function feed_table() {
			...
			$sql = "CREATE TABLE $table_name (
				...
				google_analytics smallint,
				utm_id varchar(200),
				utm_source varchar(200),
				utm_medium varchar(200),
				utm_campaign varchar(200),
				utm_source_platform varchar(200),
