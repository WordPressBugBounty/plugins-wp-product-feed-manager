...
if ( ! class_exists( 'WPPFM_Data' ) ) :
	...
	class WPPFM_Data {
		 ...
		 * 2 = On holds (will not be updated automatically)
		 * 3 = Processing
		 ...
		 *
		 * @param   string  $feed_id      The id of the feed from which the attribute data is necessary.
		...
		/**
		 * Converts feed data items that are sent through an ajax call to the corresponding database names.
		 *
		 * @param $feed_data
		 *
		 * @return array
				...
				if ( $attribute_object && ( is_object( $attribute_object ) || is_array( $attribute_object ) ) ) {
					foreach ( $attribute_object as $attribute ) {
						if ( is_array( $attribute ) && array_key_exists( 'name', $attribute ) && ! in_array( $attribute['name'], $prev_dup_array, true ) ) {
							...
							wppfm_write_log_file( sprintf( 'An attribute object could not be processed as it\'s not in an array format. The content of the attribute object = %s', $attribute ) );
						}
					}
				} else {
					if ( $attribute_object ) {
