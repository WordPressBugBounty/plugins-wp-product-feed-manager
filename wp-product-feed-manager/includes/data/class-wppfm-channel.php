	...
	class WPPFM_Channel {
		...
		public function __construct() {
			...
			$this->_channels = array(
				...
				new Wppfm_Channel_Info( '14', 'zbozi', 'Zbozi', 'https://napoveda.zbozi.cz/en/starting-to-advertise/', 'https://napoveda.zbozi.cz/en/xml-feed-2/specification/' ),
		...
		public function update_channel( $channel_short_name, $code, $nonce ) {
			if ( wp_verify_nonce( $nonce, 'update-channel-nonce' ) ) {
				...
				wppfm_write_log_file( sprintf( 'Failed to update channel %s because then nonce was not accepted. Given nonce = %s', $channel_short_name, $nonce ) );
			}
		}
		...
		public function get_number_of_channel_updates_from_server( $channel_updated ) {
			//@since 3.7.0. Only display channel updates available if the user chooses to update the channels manually
			if ( 'true' === get_option( 'wppfm_manual_channel_update', 'false' ) ) {
				return 0;
			}
		...
		private function update_channel_source( $channel_short_name, $code ) {
			...
			$file_class->delete_channel_source_files( $channel_short_name );

			$get_result = $ftp_class->get_channel_source_files( $channel_short_name, $code );

			// get the update files from wp marketingrobot.com
			if ( false !== $get_result ) {
				// unzip the file
				$file_class->unzip_channel_file( $channel_short_name );
